...
def get_filelinks():
    ...
    main_page = requests.get(MAIN_URL, verify=False).text
...
if __name__ == "__main__":
    for count, database_urls in enumerate(get_filelinks()):
        ...
        with Pool(processes=10) as pool:
            results = pool.starmap(get_file, product(database_urls, [path]))
        print ("\nDatabase {} took {} seconds.".format(count + 1, + time.time() - start_time))
